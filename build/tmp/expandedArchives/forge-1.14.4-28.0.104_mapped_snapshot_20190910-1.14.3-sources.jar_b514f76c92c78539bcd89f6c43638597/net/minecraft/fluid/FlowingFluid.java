   ...
   public Vec3d getFlow(IBlockReader p_215663_1_, BlockPos p_215663_2_, IFluidState p_215663_3_) {
      ...
      try (BlockPos.PooledMutableBlockPos blockpos$pooledmutableblockpos = BlockPos.PooledMutableBlockPos.retain()) {
         for(Direction direction : Direction.Plane.HORIZONTAL) {
            blockpos$pooledmutableblockpos.setPos(p_215663_2_).move(direction);
            IFluidState ifluidstate = p_215663_1_.getFluidState(blockpos$pooledmutableblockpos);
            if (this.isSameOrEmpty(ifluidstate)) {
               float f = ifluidstate.func_223408_f();
               float f1 = 0.0F;
               if (f == 0.0F) {
                  if (!p_215663_1_.getBlockState(blockpos$pooledmutableblockpos).getMaterial().blocksMovement()) {
                     BlockPos blockpos = blockpos$pooledmutableblockpos.down();
         ...
         if (p_215663_3_.get(FALLING)) {
            for(Direction direction1 : Direction.Plane.HORIZONTAL) {
               blockpos$pooledmutableblockpos.setPos(p_215663_2_).move(direction1);
               if (this.causesDownwardCurrent(p_215663_1_, blockpos$pooledmutableblockpos, direction1) || this.causesDownwardCurrent(p_215663_1_, blockpos$pooledmutableblockpos.up(), direction1)) {
