...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener, AutoCloseable {
   ...
   private void init() {
      ...
      this.modelManager = new ModelManager(this.textureMap, this.blockColors);
   ...
   private boolean isFramerateLimited() {
      return (double)this.getFramerateLimit() < AbstractOption.FRAMERATE_LIMIT.getMaxValue();
   }
   ...
   public CompletableFuture<Void> func_213245_w() {
      return this.supplyAsync(this::reloadResources).thenCompose((p_213240_0_) -> {
         return p_213240_0_;
      });
   }
   ...
   public static int getGLMaximumTextureSize() {
      if (cachedMaximumTextureSize == -1) {
         ...
         cachedMaximumTextureSize = MathHelper.clamp(GlStateManager.getInteger(3379), 1024, 16384);
         LOGGER.info("Failed to determine maximum texture size by probing, trying GL_MAX_TEXTURE_SIZE = {}", (int)cachedMaximumTextureSize);
      }
