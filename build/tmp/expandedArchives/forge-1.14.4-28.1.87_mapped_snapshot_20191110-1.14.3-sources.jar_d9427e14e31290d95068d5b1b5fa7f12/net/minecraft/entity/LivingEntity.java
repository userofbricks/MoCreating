...
public abstract class LivingEntity extends Entity {
   ...
   protected void updateFallState(double y, boolean onGroundIn, BlockState state, BlockPos pos) {
      ...
      if (!this.world.isRemote && this.fallDistance > 3.0F && onGroundIn) {
         ...
         if (!state.isAir(world, pos)) {
            ...
            ((ServerWorld)this.world).spawnParticle(new BlockParticleData(ParticleTypes.BLOCK, state), this.posX, this.posY, this.posZ, i, 0.0D, 0.0D, 0.0D, (double)0.15F);
         }
      }
            ...
            if (!this.canBreatheUnderwater() && !EffectUtils.canBreatheUnderwater(this) && !flag1) {
               ...
               if (this.getAir() == -20) {
                  ...
                  for(int i = 0; i < 8; ++i) {
                     ...
                     this.world.addParticle(ParticleTypes.BUBBLE, this.posX + (double)f, this.posY + (double)f1, this.posZ + (double)f2, vec3d.x, vec3d.y, vec3d.z);
                  }
   ...
   public void writeAdditional(CompoundNBT compound) {
      ...
      for(EquipmentSlotType equipmentslottype1 : EquipmentSlotType.values()) {
         ItemStack itemstack1 = this.getItemStackFromSlot(equipmentslottype1);
   ...
   protected void constructKnockBackVector(LivingEntity entityIn) {
      entityIn.knockBack(this, 0.5F, entityIn.posX - this.posX, entityIn.posZ - this.posZ);
   }
   ...
   private void renderBrokenItemStack(ItemStack stack) {
      if (!stack.isEmpty()) {
         if (!this.isSilent()) {
            this.world.playSound(this.posX, this.posY, this.posZ, SoundEvents.ENTITY_ITEM_BREAK, this.getSoundCategory(), 0.8F, 0.8F + this.world.rand.nextFloat() * 0.4F, false);
         }
   ...
   public void handleStatusUpdate(byte id) {
      ...
      case 46:
         ...
         for(int j = 0; j < 128; ++j) {
            ...
            double d1 = MathHelper.lerp(d0, this.prevPosX, this.posX) + (this.rand.nextDouble() - 0.5D) * (double)this.getWidth() * 2.0D;
            double d2 = MathHelper.lerp(d0, this.prevPosY, this.posY) + this.rand.nextDouble() * (double)this.getHeight();
            double d3 = MathHelper.lerp(d0, this.prevPosZ, this.posZ) + (this.rand.nextDouble() - 0.5D) * (double)this.getWidth() * 2.0D;
   ...
   public void dismountEntity(Entity entityIn) {
      if (!(entityIn instanceof BoatEntity) && !(entityIn instanceof AbstractHorseEntity)) {
         double d1 = entityIn.posX;
         double d13 = entityIn.getBoundingBox().minY + (double)entityIn.getHeight();
         double d14 = entityIn.posZ;
         Direction direction1 = entityIn.getAdjustedHorizontalFacing();
         if (direction1 != null) {
            Direction direction = direction1.rotateY();
            ...
            AxisAlignedBB axisalignedbb = new AxisAlignedBB(d5 - d7 / 2.0D, entityIn.getBoundingBox().minY, d6 - d8 / 2.0D, d5 + d7 / 2.0D, Math.floor(entityIn.getBoundingBox().minY) + (double)this.getHeight(), d6 + d8 / 2.0D);

            for(int[] aint : aint1) {
               double d9 = (double)(direction1.getXOffset() * aint[0] + direction.getXOffset() * aint[1]);
               double d10 = (double)(direction1.getZOffset() * aint[0] + direction.getZOffset() * aint[1]);
               double d11 = d5 + d9;
               double d12 = d6 + d10;
               AxisAlignedBB axisalignedbb1 = axisalignedbb.offset(d9, 0.0D, d10);
               if (this.world.isCollisionBoxesEmpty(this, axisalignedbb1)) {
                  BlockPos blockpos2 = new BlockPos(d11, this.posY, d12);
                  ...
                  BlockPos blockpos1 = new BlockPos(d11, this.posY - 1.0D, d12);
                  ...
                  if (this.world.isCollisionBoxesEmpty(this, axisalignedbb1.offset(0.0D, 1.0D, 0.0D)) && this.world.getBlockState(blockpos).func_215682_a(this.world, blockpos, this)) {
                     d1 = d11;
                     d13 = this.posY + 2.0D;
                     d14 = d12;
                  }
         ...
         this.setPosition(d3, entityIn.posY + (double)entityIn.getHeight() + 0.001D, d4);
         if (!this.world.isCollisionBoxesEmpty(this, this.getBoundingBox().union(entityIn.getBoundingBox()))) {
            this.setPosition(d3, entityIn.posY + (double)entityIn.getHeight() + 1.001D, d4);
            if (!this.world.isCollisionBoxesEmpty(this, this.getBoundingBox().union(entityIn.getBoundingBox()))) {
               this.setPosition(entityIn.posX, entityIn.posY + (double)this.getHeight() + 0.001D, entityIn.posZ);
            }
         }
      }
   }
   ...
   protected void jump() {
      ...
      if (this.isPotionActive(Effects.JUMP_BOOST)) {
         f = this.getJumpUpwardsMotion() + 0.1F * (float)(this.getActivePotionEffect(Effects.JUMP_BOOST).getAmplifier() + 1);
      } else {
         f = this.getJumpUpwardsMotion();
      }
         ...
         if (!this.isInWater() || this instanceof PlayerEntity && ((PlayerEntity)this).abilities.isFlying) {
            if (!this.isInLava() || this instanceof PlayerEntity && ((PlayerEntity)this).abilities.isFlying) {
               if (this.isElytraFlying()) {
                  ...
                  this.moveRelative(this.func_213335_r(f5), p_213352_1_);
               ...
               if (this.collidedHorizontally && this.isOffsetPositionInLiquid(vec3d4.x, vec3d4.y + (double)0.6F - this.posY + d7, vec3d4.z)) {
   ...
   private Vec3d func_213362_f(Vec3d p_213362_1_) {
      if (this.isOnLadder()) {
         ...
         if (d2 < 0.0D && this.getBlockState().getBlock() != Blocks.SCAFFOLDING && this.isSneaking() && this instanceof PlayerEntity) {
            d2 = 0.0D;
         }
      ...
      if (!this.world.isRemote) {
         ...
         for(EquipmentSlotType equipmentslottype : EquipmentSlotType.values()) {
            ...
            if (!ItemStack.areItemStacksEqual(itemstack1, itemstack)) {
               ...
               switch(equipmentslottype.getSlotType()) {
               case HAND:
                  this.handInventory.set(equipmentslottype.getIndex(), itemstack1.isEmpty() ? ItemStack.EMPTY : itemstack1.copy());
                  break;
               case ARMOR:
                  this.armorArray.set(equipmentslottype.getIndex(), itemstack1.isEmpty() ? ItemStack.EMPTY : itemstack1.copy());
               }
            }
         }
   ...
   public void livingTick() {
      ...
      if (this.newPosRotationIncrements > 0 && !this.canPassengerSteer()) {
         ...
         double d2 = this.posY + (this.interpTargetY - this.posY) / (double)this.newPosRotationIncrements;
         double d4 = this.posZ + (this.interpTargetZ - this.posZ) / (double)this.newPosRotationIncrements;
   ...
   public boolean attemptTeleport(double p_213373_1_, double p_213373_3_, double p_213373_5_, boolean p_213373_7_) {
      ...
      double d1 = this.posY;
      double d2 = this.posZ;
      this.posX = p_213373_1_;
      this.posY = p_213373_3_;
      this.posZ = p_213373_5_;
      boolean flag = false;
      BlockPos blockpos = new BlockPos(this);
      ...
      if (world.isBlockLoaded(blockpos)) {
         ...
         if (flag1) {
            ...
            if (world.areCollisionShapesEmpty(this) && !world.containsAnyLiquid(this.getBoundingBox())) {
               flag = true;
            }
         }
      }
...
//      });
   ...
   public ItemStack onFoodEaten(World p_213357_1_, ItemStack p_213357_2_) {
      if (p_213357_2_.isFood()) {
         p_213357_1_.playSound((PlayerEntity)null, this.posX, this.posY, this.posZ, this.getEatSound(p_213357_2_), SoundCategory.NEUTRAL, 1.0F, 1.0F + (p_213357_1_.rand.nextFloat() - p_213357_1_.rand.nextFloat()) * 0.4F);
