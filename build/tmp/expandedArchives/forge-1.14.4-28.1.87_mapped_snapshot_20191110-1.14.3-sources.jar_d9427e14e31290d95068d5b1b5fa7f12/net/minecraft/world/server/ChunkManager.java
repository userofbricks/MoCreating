...
public class ChunkManager extends ChunkLoader implements ChunkHolder.IPlayerProvider {
   ...
   private static int func_219215_b(ChunkPos pos, ServerPlayerEntity player, boolean p_219215_2_) {
      ...
      if (p_219215_2_) {
         ...
         i = MathHelper.floor(player.posX / 16.0D);
         j = MathHelper.floor(player.posZ / 16.0D);
      }
   ...
   protected ChunkHolder func_219220_a(long chunkPosIn) {
      return this.field_219251_e.get(chunkPosIn);
   }
   ...
   protected void save(boolean flush) {
      if (flush) {
         List<ChunkHolder> list = this.field_219252_f.values().stream().filter(ChunkHolder::isAccessible).peek(ChunkHolder::updateAccessible).collect(Collectors.toList());
         ...
         this.field_219252_f.values().stream().filter(ChunkHolder::isAccessible).forEach((p_222965_1_) -> {
   ...
   class EntityTracker {
      ...
      public void updateTrackingState(ServerPlayerEntity p_219400_1_) {
         if (p_219400_1_ != this.entity) {
            Vec3d vec3d = (new Vec3d(p_219400_1_.posX, p_219400_1_.posY, p_219400_1_.posZ)).subtract(this.entry.func_219456_b());
            int i = Math.min(this.range, (ChunkManager.this.viewDistance - 1) * 16);
