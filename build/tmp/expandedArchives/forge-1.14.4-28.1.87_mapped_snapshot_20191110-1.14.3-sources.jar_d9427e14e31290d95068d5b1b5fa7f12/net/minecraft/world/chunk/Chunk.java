...
public class Chunk extends net.minecraftforge.common.capabilities.CapabilityProvider<Chunk> implements IChunk, net.minecraftforge.common.extensions.IForgeChunk {
   ...
   public Chunk(World worldIn, ChunkPos p_i49945_2_, Biome[] p_i49945_3_) {
      this(worldIn, p_i49945_2_, p_i49945_3_, UpgradeData.EMPTY, EmptyTickList.get(), EmptyTickList.get(), 0L, (ChunkSection[])null, (Consumer<Chunk>)null);
   }

   public Chunk(World worldIn, ChunkPos p_i49946_2_, Biome[] p_i49946_3_, UpgradeData p_i49946_4_, ITickList<Block> p_i49946_5_, ITickList<Fluid> p_i49946_6_, long p_i49946_7_, @Nullable ChunkSection[] p_i49946_9_, @Nullable Consumer<Chunk> p_i49946_10_) {
      super(Chunk.class);
      this.entityLists = new ClassInheritanceMultiMap[16];
      this.world = worldIn;
      this.pos = p_i49946_2_;
      this.upgradeData = p_i49946_4_;
      ...
      this.blockBiomeArray = p_i49946_3_;
      this.blocksToBeTicked = p_i49946_5_;
      this.fluidsToBeTicked = p_i49946_6_;
      this.inhabitedTime = p_i49946_7_;
      this.field_217330_v = p_i49946_10_;
      if (p_i49946_9_ != null) {
         if (this.sections.length == p_i49946_9_.length) {
            System.arraycopy(p_i49946_9_, 0, this.sections, 0, this.sections.length);
         } else {
    ...
    */
   public void getEntitiesWithinAABBForList(@Nullable EntityType<?> entitytypeIn, AxisAlignedBB aabb, List<Entity> list, Predicate<? super Entity> filter) {
      int i = MathHelper.floor((aabb.minY - this.world.getMaxEntityRadius()) / 16.0D);
      int j = MathHelper.floor((aabb.maxY + this.world.getMaxEntityRadius()) / 16.0D);
      i = MathHelper.clamp(i, 0, this.entityLists.length - 1);
      j = MathHelper.clamp(j, 0, this.entityLists.length - 1);
      ...
      for(int k = i; k <= j; ++k) {
         for(Entity entity : this.entityLists[k].func_219790_a(Entity.class)) {
            if ((entitytypeIn == null || entity.getType() == entitytypeIn) && entity.getBoundingBox().intersects(aabb) && filter.test(entity)) {
               list.add(entity);
            }
         }
      }
   ...
   /**
    * Loads this chunk from the given buffer and NBT
    */
   @OnlyIn(Dist.CLIENT)
   public void read(PacketBuffer buffer, CompoundNBT nbt, int availableSections, boolean isFullChunk) {
      Predicate<BlockPos> predicate = isFullChunk ? (p_217315_0_) -> {
         return true;
      } : (p_217323_1_) -> {
      ...
      for(int i = 0; i < this.sections.length; ++i) {
         ...
         if ((availableSections & 1 << i) == 0) {
            if (isFullChunk && chunksection != EMPTY_SECTION) {
      ...
      if (isFullChunk) {
         for(int j = 0; j < this.blockBiomeArray.length; ++j) {
            this.blockBiomeArray[j] = Registry.BIOME.getByValue(buffer.readInt());
         }
      }
      ...
      for(Heightmap.Type heightmap$type : Heightmap.Type.values()) {
         ...
         if (nbt.contains(s, 12)) {
            this.setHeightmap(heightmap$type, nbt.getLongArray(s));
         }
      }
