...
public abstract class PlayerList {
   ...
   public void initializeConnectionToPlayer(NetworkManager netManager, ServerPlayerEntity playerIn) {
      ...
      serverplaynethandler.sendPacket(new SJoinGamePacket(playerIn.getEntityId(), playerIn.interactionManager.getGameType(), worldinfo.isHardcore(), serverworld.dimension.getType(), this.getMaxPlayers(), worldinfo.getGenerator(), this.viewDistance, serverworld.getGameRules().getBoolean(GameRules.REDUCED_DEBUG_INFO)));
      ...
      serverplaynethandler.setPlayerLocation(playerIn.posX, playerIn.posY, playerIn.posZ, playerIn.rotationYaw, playerIn.rotationPitch);
   ...
   public ServerPlayerEntity recreatePlayerEntity(ServerPlayerEntity playerIn, DimensionType dimension, boolean conqueredEnd) {
      ...
      while(!serverworld.areCollisionShapesEmpty(serverplayerentity) && serverplayerentity.posY < 256.0D) {
   ...
   public void sendToAllNearExcept(@Nullable PlayerEntity except, double x, double y, double z, double radius, DimensionType dimension, IPacket<?> packetIn) {
      for(int i = 0; i < this.players.size(); ++i) {
         ...
         if (serverplayerentity != except && serverplayerentity.dimension == dimension) {
            double d0 = x - serverplayerentity.posX;
            double d1 = y - serverplayerentity.posY;
            double d2 = z - serverplayerentity.posZ;
